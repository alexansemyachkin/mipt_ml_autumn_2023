...
class SoftMax(Module):
    ...
    def updateOutput(self, input):
        ...
        self.output = np.divide(exp_input, exp_input.sum(axis=1, keepdims=True))
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        self.gradInput = -self.output * np.sum(self.output * gradOutput, axis=1, keepdims=True) + self.output * gradOutput
...
class LogSoftMax(Module):
    ...
    def updateOutput(self, input):
        ...
        # Your code goes here. ################################################
        exp_input = np.exp(self.output)
        log_sum_exp = np.log(exp_input.sum(axis=1, keepdims=True))
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        self.gradInput = -softmax_output * np.sum(gradOutput, axis=1, keepdims=True) + gradOutput
...
class BatchNormalization(Module):
    ...
    def updateOutput(self, input):
        ...
        if self.training:
            ...
            self.normalized_input = (input - self.mean) / np.sqrt(self.variance + self.EPS)
        else:
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        term1 = batch_size * gradOutput
        term2 = np.sum(gradOutput, axis=0)
        term3 = np.sum(gradOutput * self.output, axis=0) * self.output
        self.gradInput = (1.0 / (batch_size * np.sqrt(variance + self.EPS))) * (term1 - term2 - term3)
...
class LeakyReLU(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        self.gradInput = np.multiply(gradOutput, input > 0) + np.multiply(self.slope * gradOutput, input <= 0)
...
class ELU(Module):
    ...
    def updateOutput(self, input):
        # Your code goes here. ################################################
        self.output = np.multiply(input, input > 0) + np.multiply(self.alpha * (np.exp(input) - 1), input <= 0)
        
        return self.output
    
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        self.gradInput = np.multiply(gradOutput, input > 0) + np.multiply(gradOutput * self.alpha * np.exp(input), input <= 0)
...
class SoftPlus(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        self.gradInput = gradOutput * exp_input / (1 + exp_input)
        return self.gradInput
